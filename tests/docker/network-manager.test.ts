...
describe('NetworkManager', () => {
  ...
  beforeEach(() => {
    ...
    mockDocker.createNetwork = jest.fn().mockResolvedValue({
      id: 'test-network-id',
    });
  ...
  describe('Initialization', () => {
    ...
    it('should handle initialization errors gracefully', async () => {
      mockDocker.info = jest.fn().mockRejectedValue(new Error('Docker connection failed'));
  ...
  describe('Task Network Creation', () => {
    ...
    it('should configure IPAM with subnet', async () => {
      ...
      expect(createCall.ipam?.config?.[0]?.subnet).toMatch(/^172\.28\.\d+\.0\/24$/);
    });

    it('should throw OpenCodeError on creation failure', async () => {
      const taskId = 'test-task-6';
      mockDocker.createNetwork = jest.fn().mockRejectedValue(new Error('Network creation failed'));

      await expect(networkManager.createTaskNetwork(taskId)).rejects.toThrow(OpenCodeError);
    });
  });
  ...
  describe('Container Connection', () => {
    ...
    it('should connect with custom endpoint config', async () => {
      ...
      await networkManager.connectContainerToNetwork(containerId, networkId, endpointConfig);
    ...
    it('should throw OpenCodeError on connection failure', async () => {
      mockNetwork.connect = jest.fn().mockRejectedValue(new Error('Connection failed'));
  ...
  describe('Container Disconnection', () => {
    ...
    it('should disconnect container from network successfully', async () => {
      await networkManager.disconnectContainerFromNetwork(containerId, networkId);
    ...
    it('should throw OpenCodeError on other errors', async () => {
      mockNetwork.disconnect = jest.fn().mockRejectedValue(new Error('Disconnection failed'));
  ...
  describe('Network Removal', () => {
    ...
    it('should handle non-existent networks gracefully', async () => {
      ...
      await expect(networkManager.removeTaskNetwork(nonExistentTaskId)).resolves.not.toThrow();
    });
    ...
    it('should handle 404 errors during removal', async () => {
      ...
      await expect(networkManager.removeTaskNetwork(taskId)).resolves.not.toThrow();
    });

    it('should throw OpenCodeError on other errors', async () => {
      mockNetwork.remove = jest.fn().mockRejectedValue(new Error('Removal failed'));

      await expect(networkManager.removeTaskNetwork(taskId)).rejects.toThrow(OpenCodeError);
    });
  });
  ...
  describe('Network Listing', () => {
    ...
    it('should return empty array for tasks with no networks', async () => {
      const networks = await networkManager.listTaskNetworks('non-existent-task');
  ...
  describe('Error Codes', () => {
    ...
    it('should use NETWORK_INITIALIZATION_FAILED for init errors', async () => {
      ...
      await expect(newManager.initialize()).rejects.toMatchObject({
        code: ErrorCode.NETWORK_INITIALIZATION_FAILED,
      });
    });

    it('should use NETWORK_CREATION_FAILED for creation errors', async () => {
      const newTaskId = 'new-task-error';
      mockDocker.createNetwork = jest.fn().mockRejectedValue(new Error('Creation failed'));

      await expect(networkManager.createTaskNetwork(newTaskId)).rejects.toMatchObject({
        code: ErrorCode.NETWORK_CREATION_FAILED,
      });
    });
    ...
    it('should use NETWORK_DISCONNECTION_FAILED for disconnection errors', async () => {
      ...
      await expect(
        ...
        code: ErrorCode.NETWORK_DISCONNECTION_FAILED,
      });
    });
  });
